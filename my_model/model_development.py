...
DYNAMIC_STRENGTH = 4.5  # Multiplier for weight adjustments
...
def compute_asymmetric_extreme_boost(mvrv_zscore: np.ndarray) -> np.ndarray:
    ...
    # Danger zone: VERY strong negative boost (quadratic decrease)
    # 根据EDA发现：Danger zone虽然短期回报高，但是周期顶部信号，应该大幅减少购买
    danger_mask = mvrv_zscore >= MVRV_ZONE_DANGER
    boost = np.where(
        danger_mask,
        -1.0 * (mvrv_zscore - MVRV_ZONE_DANGER) ** 2 - 0.8,  # 加倍减少幅度
        boost,
    )
...
def compute_dynamic_multiplier(
    ...
    # Primary: MVRV value (70%), Secondary: MA (30%), No Polymarket
    # 根据EDA发现：移除Polymarket，将权重重新分配给MVRV和MA200
    combined = value_signal * 0.70 + ma_signal * 0.30
