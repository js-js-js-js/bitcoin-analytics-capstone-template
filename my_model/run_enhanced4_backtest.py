...
# Import Enhanced4 Model
from my_model.model_development_enhanced4 import precompute_features, compute_window_weights
...
def compute_weights_wrapper(df_window: pd.DataFrame) -> pd.Series:
    """Wrapper for Enhanced4 compute_window_weights.

    Adapts the specific Enhanced4 model function to the interface expected
...
def main():
    ...
    logger.info("Starting Enhanced4 Bitcoin DCA Strategy Analysis")

    # Load data
    btc_df = load_data()

    # Precompute features
    logger.info("Precomputing enhanced4 features...")
    _FEATURES_DF = precompute_features(btc_df)

    # Run backtest
    logger.info("Running SPD backtest for 'Enhanced4 Model'...")

    # Create output directory
    output_dir = Path("my_model/output_enhanced4")
    ...
    run_full_analysis(
        ...
        strategy_label="Enhanced4 Model",
    )
